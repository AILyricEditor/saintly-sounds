...
export default function Navbar() {
	...
	const links = [
		{ 
			...
			active: <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e3e3e3"><path d="M160-120v-480l320-240 320 240v480H560v-280H400v280H160Z"/></svg>
		},
		{ 
		...
		{ 
			...
			sublinks: [
				{ href: '/music/', label: 'Genre' },
				{ href: '/music/', label: 'Artist' },
				{ href: '/music/', label: 'Album' },
				{ href: '/music/', label: 'Song' },
			]
		},
		{ href: '/',
			label: 'News',
					...
					{links.map((link, index) => {
						return (
							<Link className={`
								...
								key={index}
							>
								{link.sublinks && <div className={styles.sublinks}>
									{link.sublinks.map((link, index) => {
										return (
											<Link className={`
													${pathname === link.href ? styles.active : ''} 
													${styles.tab} ${styles.sublink}
												`}
												href={link.href} 
												key={index}
											>	
